...
function DonorNotification() {
  ...
  const navigateForNotification = (n) => {
    // Map donor-side notifications to target pages
    switch (n.type) {
      ...
      case "request_created":
        window.location.href = "/donor/showrequests";
        break;
      case "event_created":
  ...
  const handleMarkAll = async () => {
    ...
    try {
      ...
      // refetch to avoid UI/server mismatch
      const res = await getNotifications(token);
      setNotifications(res.data);
    } catch (e) {
  ...
  return (
    <div className="font-serif text-green-800">
      ...
      <div className="font-serif text-green-800 mt-24 ml-60">
